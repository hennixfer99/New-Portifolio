    ...
    <Container>
      {width > breakpoint ? (
        <ul className="buttons-homepage">
          ...
          <button onClick={github}><img src={gh} alt="github"/></button>
          
          <button onClick={linkedin}><img src={lk} alt="linkedin"/></button>
          
        </ul>
      ) : (
        <figure>
          <img className="menu" alt="logo" src={menu} onClick={menuAtivo} />
        </figure>
      )}

      <Menu abrir={menuOpen} sair={setMenuOpen} link1={github} link2={linkedin}/>
    </Container>
  );
}
